...
svm_atomic<int> *ML_Base::xvernumber = NULL;
svm_atomic<int> *ML_Base::gvernumber = NULL;
...
{
    {
        ...
        if ( xvernumber == NULL )
        {
            xvernumber = new svm_atomic<int>[NUMMLINSTANCES];
            gvernumber = new svm_atomic<int>[NUMMLINSTANCES];
...
int ML_Base::setMLid(int nv)
{
    ...
    if ( !res )
    {
        NiceAssert( xvernumber[nv] == 0 );
        NiceAssert( gvernumber[nv] == 0 );

        int xvn = xvernumber[oldMLid];
        int gvn = gvernumber[oldMLid];

        xvernumber[oldMLid] = 0; // Indicate no longer present
        gvernumber[oldMLid] = 0; // Indicate no longer present
        ...
        if ( indexKeyCount(i) )
        {
            ...
            for ( j = 0 ; ( j < ML_Base::N() ) && indexKeyCount(i) ; j++ )
            {
                ...
                for ( n = 0 ; n < m ; n++ )
                {
                    ...
                    if ( xx.isnearindpresent(indexKey(i),n) )
                    {
...
void ML_Base::dgTrainingVector(Vector<gentype> &res, const Vector<int> &i) const
{
    ...
    {
        static gentype zerotemplate(0.0);
...
int ML_Base::getparam(int ind, gentype &val, const gentype &xa, int ia, const gentype &xb, int ib) const
{
    ...
    if ( ( ind <= 499 ) && ( !ia && !ib ) )
    {
        ...
        switch ( ind )
        {
            ...
            case 303: { Vector<double> res; dgTrainingVectorX(res,(int) xa); val = res;  break; }
    ...
    else if ( ( ind <= 599 ) && !ib )
    {
        ...
        else
        {
            ...
            switch ( ind )
            {
                ...
                case 503: { Vector<double> res; dgX(res,xx); val = res;  break; }
                case 504: { Vector<gentype> res; gentype resn; dg(res,resn,xx); val = res;  break; }
                case 505: { Vector<gentype> res; gentype resn; dg(res,resn,xx); val = resn; break; }
