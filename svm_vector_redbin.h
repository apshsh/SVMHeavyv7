...
public:
    ...
    virtual const Vector<Vector<double> >      &zV         (void)      const { return traintarg;                                   }
    ...
    virtual const Vector<double> &zV(int i) const { return ( i >= 0 ) ? zV()(i) : ((const Vector<double> &) y(i)); }
    ...
    virtual double         &dedgTrainingVector(double         &res, int i) const { return ML_Base::dedgTrainingVector(res,i); }
    virtual Vector<double> &dedgTrainingVector(Vector<double> &res, int i) const;
    virtual d_anion        &dedgTrainingVector(d_anion        &res, int i) const { return ML_Base::dedgTrainingVector(res,i); }
    virtual gentype        &dedgTrainingVector(gentype        &res, int i) const { Vector<double> tmp(tspaceDim()); return ( res = dedgTrainingVector(tmp,i) ); }

    virtual double dedgAlphaTrainingVector(int i, int j) const { Vector<double> tmp(tspaceDim()); double res; return twoProduct(res,dedgTrainingVector(tmp,i),alphaV()(j)); }
    virtual Matrix<double> &dedgAlphaTrainingVector(Matrix<double> &res) const;
    ...
    virtual void dgTrainingVectorX(Vector<gentype> &resx, const Vector<int> &i) const { SVM_Generic::dgTrainingVectorX(resx,i); return; }
    virtual void dgTrainingVectorX(Vector<double>  &resx, const Vector<int> &i) const { SVM_Generic::dgTrainingVectorX(resx,i); return; }
...
template <class BaseRegressorClass>
Matrix<double> &SVM_Vector_redbin<BaseRegressorClass>::dedgAlphaTrainingVector(Matrix<double> &res) const
{
    res.resize(N(),N());
    ...
    for ( i = 0 ; i < N() ; i++ )
    {
        if ( alphaState()(i) )
        {
            ...
            for ( j = 0 ; j < N() ; j++ )
            {
                if ( alphaState()(j) )
                {
                    twoProduct(res("&",i,j),tmp,alphaV()(j));
                }
            }
        }
    }
