...
double LSV_Vector::eTrainingVector(int i) const
{
    double res = 0;
...
gentype &LSV_Vector::dedgTrainingVector(gentype &res, int i) const
{
    ...
    {
        static gentype zeroval(0.0);

        res.force_vector() = zeroval;
    }

    return res;
}

Matrix<double> &LSV_Vector::dedgAlphaTrainingVector(Matrix<double> &res) const
{
    res.resize(N(),N());
