...
//    virtual const Vector<gentype>                &y          (void) const { return getRepConst().y();           }
    ...
    virtual const gentype &y(int i)                           const { return ( i >= 0 ) ? y()(i) : getRepConst().y(i); }
    ...
    virtual void dgX(Vector<gentype> &resx, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const;
    virtual void dgX(Vector<double>  &resx, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const;

    virtual int gg(double &resg,         const SparseVector<gentype> &x, const vecInfo *xinf = NULL, gentype ***pxyprodx = NULL) const { gentype resgg; int tmp = gg(resgg,x,xinf,pxyprodx); resg = (double)                 resgg; return tmp; }
    virtual int gg(Vector<double> &resg, const SparseVector<gentype> &x, const vecInfo *xinf = NULL, gentype ***pxyprodx = NULL) const { gentype resgg; int tmp = gg(resgg,x,xinf,pxyprodx); resg = (const Vector<double> &) resgg; return tmp; }
    virtual int gg(d_anion &resg,        const SparseVector<gentype> &x, const vecInfo *xinf = NULL, gentype ***pxyprodx = NULL) const { gentype resgg; int tmp = gg(resgg,x,xinf,pxyprodx); resg = (const d_anion &)        resgg; return tmp; }

    virtual void dg(Vector<gentype>         &res, gentype        &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const;
    virtual void dg(Vector<double>          &res, double         &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const;
    virtual void dg(Vector<Vector<double> > &res, Vector<double> &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const;
    virtual void dg(Vector<d_anion>         &res, d_anion        &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const;

    virtual void stabProb(double  &res, const SparseVector<gentype> &x, int p, double pnrm, int rot, double mu, double B) const;
    ...
    virtual int setSampleMode(int nv, const Vector<gentype> &xmin, const Vector<gentype> &xmax, int Nsamp = DEFAULT_SAMPLES_SAMPLE, int sampSplit = 1, int sampType = 0)
    {
        int res = 0;
        int i;
        ...
        for ( i = 0 ; i < numReps() ; i++ )
        {
            res |= getRep(i).setSampleMode(nv,xmin,xmax,Nsamp,sampSplit,sampType);
        }

        return res | BLK_Generic::setSampleMode(nv,xmin,xmax,Nsamp,sampSplit,sampType);
    }
