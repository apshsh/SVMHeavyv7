...
//        }
    ...
    d_anion                &force_anion (int i = -1)           {                                                       if ( !isValAnion()   ) { *this = zeroanion(); } if ( i >= 0 ) { (*anionval).setorder(i);  }  return *anionval; }
    ...
    Matrix<gentype>        &force_matrix(int i = 0, int j = 0) { const static Matrix<gentype>        defaultval;       if ( !isValMatrix()  ) { *this = defaultval;  } if ( i >= 0 ) { (*matrixval).resize(i,j); } return *matrixval; }
