...
int addtrainingdata(ML_Base &mlbase, const char *trainfile, int reverse, int ignoreStart, int imax, int ibase)
{
    std::string tfile(trainfile);
    SparseVector<gentype> xtemp;

    return addtrainingdata(mlbase,xtemp,tfile,reverse,ignoreStart,imax,ibase);
}

int loadFileAndTest(const ML_Base &mlbase, const char *trainfile, int reverse, int ignoreStart, int imax, int ibase, 
                    int coercetosingle, int coercefromsingle, int fromsingletarget, int binaryRelabel, int singleDrop)
{
    std::string tfile(trainfile);
    SparseVector<gentype> xtemp;
    ...
    return loadFileAndTest(mlbase,xtemp,tfile,reverse,ignoreStart,imax,ibase,
                           ...
                           0,linesread,ytest,ytestresh,ytestresg,gvarres,0,outkernind);
}
