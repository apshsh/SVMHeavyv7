...
// These are handy if available, but not strictly necessary

#ifdef IS_CPP11
#define svm_explicit explicit
#define svm_override override
#endif
#ifndef IS_CPP11
#define svm_explicit
#define svm_override
...
// ------------------------------------------------------------------------
//
// Optimised maths code (AVX2 optional)
//
...
inline int svm_recvfrom(int a, char *b, int c, int d, svm_sockaddr *e, svm_socklen_t *f);
inline int svm_recvfrom(int a, char *b, int c, int d, svm_sockaddr *e, svm_socklen_t *f) { (void) a; (void) b; (void) c; (void) d; (void) e; (void) f; return -1; }
...
inline int svm_accept(int a, svm_sockaddr *b, svm_socklen_t *c);
inline int svm_accept(int a, svm_sockaddr *b, svm_socklen_t *c) { (void) a; (void) b; (void) c; return -1; }
...
//
// - data type is svm_recursive_mutex
// - svm_mutex_lock: locks the mutex if available, blocks if mutex owned
//   by another thread
// - svm_mutex_unlock: unlocks mutex.  Mutexes must be unlocked by the
//   same thread that they were locked in.  If multiple lockes then require
//   same number of unlocks (by owning thread) to unlock the mutex.
//
// ------------------------------------------------------------------------
// ------------------------------------------------------------------------
// ------------------------------------------------------------------------
...
// Atomics:
//
// - some bits from std::atomic (good for c++11, otherwise flakey/slow)
//
// ------------------------------------------------------------------------
// ------------------------------------------------------------------------
// ------------------------------------------------------------------------
...
#define svmvolatilemutex

typedef std::mutex svm_mutex;
typedef std::recursive_mutex svm_recursive_mutex;

#define svm_mutex_lock(thismut)   thismut.lock()
#define svm_mutex_unlock(thismut) thismut.unlock()
...
#define svmvolatilemutex volatile
...
public:
    ...
    svm_recursive_mutex()
    {
        ...
        int resinit = pthread_mutex_init(const_cast<pthread_mutex_t *>(&actualMutex),&Attr);
        (void) resinit;
        NiceAssert( !resinit );
        //int resunlo = pthread_mutex_unlock(const_cast<pthread_mutex_t *>(&actualMutex));
        //(void) resunlo;
...
inline void svm_mutex_lock(svmvolatilemutex svm_mutex &temp);
inline void svm_mutex_lock(svmvolatilemutex svm_mutex &temp)
{
    pthread_mutex_lock(const_cast<pthread_mutex_t *>(&(temp.actualMutex)));
}

inline void svm_mutex_unlock(svmvolatilemutex svm_mutex &temp);
inline void svm_mutex_unlock(svmvolatilemutex svm_mutex &temp)
{
    pthread_mutex_unlock(const_cast<pthread_mutex_t *>(&(temp.actualMutex)));
}

inline void svm_mutex_lock(svmvolatilemutex svm_recursive_mutex &temp);
inline void svm_mutex_lock(svmvolatilemutex svm_recursive_mutex &temp)
{
    pthread_mutex_lock(const_cast<pthread_mutex_t *>(&(temp.actualMutex)));
}

inline void svm_mutex_unlock(svmvolatilemutex svm_recursive_mutex &temp);
inline void svm_mutex_unlock(svmvolatilemutex svm_recursive_mutex &temp)
{
    pthread_mutex_unlock(const_cast<pthread_mutex_t *>(&(temp.actualMutex)));
}
...
class svm_recursive_mutex
{
    ...
    svm_mutex() { lockon = 0; overthread = 0; }

    int lockon;
    svm_pthread_id ownerthread;
};
...
inline void svm_mutex_lock(svmvolatilemutex svm_recursive_mutex &temp)
{
    if ( temp.lockon )
    {
        if ( !svm_pthread_same_as_this(ownerthread) )
        {
            while ( temp.lockon )
            {
                ;
...
typedef pthread_t svm_pthread_id;
...
inline int svm_pthread_create(svm_pthread_t *a, void *(*c)(void *), void *d)
{
    return pthread_create(a,NULL,c,d);
}
...
inline int svm_pthread_same_as_this(const svm_pthread_id &a)
{
    return pthread_equal(a,pthread_self());
}
...
inline int svm_pthread_same_as_this(const svm_pthread_id &a)
{
    return ( a.thread_id == GetCurrentThreadId() );
}
...
class svm_atomic
{
    ...
    svm_atomic(const T &val)
    {
        contents = val;
    }

    svm_atomic(const svm_atomic &src)
    {
...
#define svm_mutex int
#define svm_recursive_mutex int

#define svm_mutex_lock(thismut) (void) thismut
#define svm_mutex_unlock(thismut) (void) thismut

typedef int svm_pthread_t;
typedef int svm_pthread_id;

inline int threads_enabled(void);
inline int threads_enabled(void) { return 0; }

inline int svm_pthread_create(svm_pthread_t *a, void *(*c)(void *), void *d);
inline int svm_pthread_create(svm_pthread_t *a, void *(*c)(void *), void *d) { (void) c; (void) d; static svm_pthread_t onlythreadid = 0; *a = onlythreadid; return -1; }

inline svm_pthread_id svm_pthread_self(void);
inline svm_pthread_id svm_pthread_self(void) { return 0; }

inline int svm_pthread_same_as_this(const svm_pthread_id &a);
inline int svm_pthread_same_as_this(const svm_pthread_id &a) { (void) a; return 1; }
...
inline int genUUID(void)
{
    ...
    static svm_atomic<int> nextUUID(1);
