...
public:
    ...
    virtual Vector<double> &dedgTrainingVector(Vector<double> &res, int i) const { return ML_Base::dedgTrainingVector(res,i); }
    virtual d_anion        &dedgTrainingVector(d_anion        &res, int i) const { return ML_Base::dedgTrainingVector(res,i); }
    virtual gentype        &dedgTrainingVector(gentype        &res, int i) const;

    virtual double dedgAlphaTrainingVector(int i, int j) const { d_anion tmp(order()); d_anion resg(order()); return twoProduct(resg,dedgTrainingVector(tmp,i),alphaA()(j)).realpart(); }
