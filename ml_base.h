...
public:
    ...
    virtual const int *ClassLabelsInt(void) const { return &(getMLconst().ClassLabels()(zeroint())); }
    ...
    virtual void npCweight    (double **res, int *dim) const { *dim = Cweight().size();     *res = const_cast<double *>(&(Cweight()(zeroint())));     return; }
    virtual void npCweightfuzz(double **res, int *dim) const { *dim = Cweightfuzz().size(); *res = const_cast<double *>(&(Cweightfuzz()(zeroint()))); return; }
    virtual void npsigmaweight(double **res, int *dim) const { *dim = sigmaweight().size(); *res = const_cast<double *>(&(sigmaweight()(zeroint()))); return; }
    virtual void npepsweight  (double **res, int *dim) const { *dim = epsweight().size();   *res = const_cast<double *>(&(epsweight()(zeroint())));   return; }
    ...
    virtual int xvernum(void)        const { return xvernumber[MLid()];   }
    virtual int xvernum(int altMLid) const { return xvernumber[altMLid];  }
    virtual int incxvernum(void)           { return ++xvernumber[MLid()]; }
    virtual int gvernum(void)        const { return gvernumber[MLid()];   }
    virtual int gvernum(int altMLid) const { return gvernumber[altMLid];  }
    virtual int incgvernum(void)           { return ++gvernumber[MLid()]; }
    ...
    //
    // Setting this 2 does pre-calculations for sampling but does not
    // take the actual sample.  This can save on calculations if you
    // pre-sample a source and, make lots of copies and sample the copies.
    //
    ...
    virtual int setSampleMode(int nv, const Vector<gentype> &xmin, const Vector<gentype> &xmax, int Nsamp = DEFAULT_SAMPLES_SAMPLE, int sampSplit = 1, int sampType = 0) { (void) nv; (void) xmin; (void) xmax; (void) Nsamp; (void) sampSplit; (void) sampType; return 0; }
    ...
    // dedgAlpha: dedg multiplied by alpha (always a double)
    // dedgAlpha: matrix verison only fills in those spots for which alphaState()(i) and alphaState()(j) are nonzero
    //
    ...
    virtual double eTrainingVector(int i) const { (void) i; throw("eTrainingVector not implemented for this ML"); return 0.0; }
    ...
    virtual gentype        &dedgTrainingVector(gentype        &res, int i) const { (void) res; (void) i; throw("dedgTrainingVector not implemented for this ML"); return res; }

    virtual double dedgAlphaTrainingVector(int i, int j) const { (void) i; (void) j; throw("dedgalphaTrainingVector not implemented for this ML"); return 0.0; }
    virtual Matrix<double> &dedgAlphaTrainingVector(Matrix<double> &res) const { throw("dedgalphaTrainingVector not implemented for this ML"); return res; }
    ...
    virtual int ggTrainingVector(Vector<double> &resg, int i, int retaltg = 0, gentype ***pxyprodi = NULL) const { (void) pxyprodi; gentype res; int resi = ggTrainingVector(res,i,retaltg); resg = (const Vector<double> &) res; return resi; }
    ...
    virtual void deTrainingVector(Vector<gentype> &res, gentype &resn, int i) const { dgTrainingVector(res,resn,i); double scale = 0.0; dedgTrainingVector(scale,i); res.scale(scale); resn *= scale; return; }
    ...
    virtual double e(const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); double res = eTrainingVector(-1); resetInnerWildp(xinf == NULL); return res; }
    ...
    virtual void dedg(double         &res, const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); dedgTrainingVector(res,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dedg(Vector<double> &res, const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); dedgTrainingVector(res,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dedg(d_anion        &res, const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); dedgTrainingVector(res,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dedg(gentype        &res, const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); dedgTrainingVector(res,-1); resetInnerWildp(xinf == NULL); return; }

    virtual void dgX(Vector<gentype> &resx, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); dgTrainingVectorX(resx,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dgX(Vector<double>  &resx, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); dgTrainingVectorX(resx,-1); resetInnerWildp(xinf == NULL); return; }

    virtual void deX(Vector<gentype> &resx, const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); deTrainingVectorX(resx,-1); resetInnerWildp(xinf == NULL); return; }
    ...
    virtual void dg(Vector<gentype>         &res, gentype        &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); dgTrainingVector(res,resn,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dg(Vector<double>          &res, double         &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); dgTrainingVector(res,resn,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dg(Vector<Vector<double> > &res, Vector<double> &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); dgTrainingVector(res,resn,-1); resetInnerWildp(xinf == NULL); return; }
    virtual void dg(Vector<d_anion>         &res, d_anion        &resn, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); dgTrainingVector(res,resn,-1); resetInnerWildp(xinf == NULL); return; }

    virtual void de(Vector<gentype> &res, gentype &resn, const gentype &y, const SparseVector<gentype> &x, const vecInfo *xinf = NULL) const { setInnerWildpa(&x,xinf); setWildTargpp(y); deTrainingVector(res,resn,-1); resetInnerWildp(xinf == NULL); return; }
...
//FIXME    Vector<const vecInfo *> traininfop;
    ...
    static svm_atomic<int> *xvernumber;
    static svm_atomic<int> *gvernumber;
