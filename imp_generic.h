...
public:
    ...
    virtual       ML_Base &getML(void)            svm_override { return static_cast<      ML_Base &>(getIMP()     ); }
    virtual const ML_Base &getMLconst(void) const svm_override { return static_cast<const ML_Base &>(getIMPconst()); }

    // Information functions (training data):

    virtual int subtype(void) const svm_override { return 0;   }

    virtual int isTrained(void) const svm_override { return disTrained; }

    virtual char targType(void) const svm_override { return 'N'; }
    ...
    virtual int setx(int                i, const SparseVector<gentype>          &x) svm_override;
    virtual int setx(const Vector<int> &i, const Vector<SparseVector<gentype> > &x) svm_override;
    virtual int setx(                      const Vector<SparseVector<gentype> > &x) svm_override;

    virtual int qswapx(int                i, SparseVector<gentype>          &x, int dontupdate = 0) svm_override;
    virtual int qswapx(const Vector<int> &i, Vector<SparseVector<gentype> > &x, int dontupdate = 0) svm_override;
    virtual int qswapx(                      Vector<SparseVector<gentype> > &x, int dontupdate = 0) svm_override;
    ...
    virtual int train(int &res, svmvolatile int &) svm_override { (void) res; disTrained = 1; return 0; }
    virtual int train(int &res) svm_override { svmvolatile int killSwitch = 0; return train(res,killSwitch); }

    // General modification and autoset functions

    virtual int reset(void)   svm_override { untrain(); return 1;       }
